...
/// Identity columns must round-trip: GENERATED ... AS IDENTITY is part of the
/// column, and the identity-owned sequence is internal — emitting it as a
/// standalone sequence (or dropping the identity clause) produces schemas
/// whose inserts fail with NOT NULL violations.
#[tokio::test]
async fn test_create_table_with_identity_columns() -> Result<()> {
    let helper = MigrationTestHelper::new().await;
        ...
        .run_migration_test(
            ...
            |steps, final_catalog| {
                ...
                assert_eq!(
                    ...
                    "rental_id should be GENERATED BY DEFAULT AS IDENTITY"
                );
                assert_eq!(create_step[1].identity, Some(IdentityKind::Always));
                assert_eq!(create_step[2].identity, None, "serial is not identity");

                let table = &final_catalog.tables[0];
                assert_eq!(table.columns[0].identity, Some(IdentityKind::ByDefault));
                assert_eq!(table.columns[0].default, None, "identity has no DEFAULT");
                assert_eq!(table.columns[1].identity, Some(IdentityKind::Always));
                assert_eq!(table.columns[2].identity, None);
                assert!(
                    table.columns[2]
                        .default
                        .as_deref()
                        .is_some_and(|d| d.contains("nextval")),
                    "serial keeps its nextval default"
                );

                // Identity-owned sequences are internal to the column; only the
                // serial's sequence is a standalone catalog object.
                assert_eq!(
                    final_catalog
                        .sequences
                        .iter()
                        .map(|s| s.name.as_str())
                        .collect::<Vec<_>>(),
                    vec!["rental_legacy_id_seq"],
                    "identity sequences must not appear as standalone objects"
                );
...
#[tokio::test]
async fn test_add_identity_to_existing_column() -> Result<()> {
    let helper = MigrationTestHelper::new().await;
    ...
    helper
        .run_migration_test(
            ...
            &["CREATE TABLE items (id bigint GENERATED BY DEFAULT AS IDENTITY)"],
            |steps, final_catalog| {
                use pgmt::catalog::table::IdentityKind;
                    ...
                    steps.iter().any(|s| matches!(
                        ...
                        MigrationStep::Table(TableOperation::Alter { actions, .. })
                            if actions.iter().any(|a| matches!(
                                a,
                                ColumnAction::AddIdentity { name, kind }
                                    if name == "id" && *kind == IdentityKind::ByDefault
                            ))
                    )),
                    "Should have AddIdentity action, got: {:?}",
                    steps
                );

                assert_eq!(
                    final_catalog.tables[0].columns[0].identity,
                    Some(IdentityKind::ByDefault)
                );
                Ok(())
            },
...
#[tokio::test]
async fn test_drop_identity_migration() -> Result<()> {
    let helper = MigrationTestHelper::new().await;

    helper
        .run_migration_test(
            &[],
            &["CREATE TABLE items (id bigint GENERATED ALWAYS AS IDENTITY)"],
            &["CREATE TABLE items (id bigint NOT NULL)"],
            |steps, final_catalog| {
                assert!(
                    steps.iter().any(|s| matches!(
                        ...
                        MigrationStep::Table(TableOperation::Alter { actions, .. })
                            if actions.iter().any(|a| matches!(
                                a,
                                ColumnAction::DropIdentity { name } if name == "id"
                            ))
                    )),
                    "Should have DropIdentity action, got: {:?}",
                    steps
                );

                assert_eq!(final_catalog.tables[0].columns[0].identity, None);
                assert!(
                    final_catalog.tables[0].columns[0].not_null,
                    "column stays NOT NULL after identity is dropped"
                );
                Ok(())
            },
        )
...
#[tokio::test]
async fn test_change_identity_kind_migration() -> Result<()> {
    let helper = MigrationTestHelper::new().await;
                ...
                assert!(
                    steps.iter().any(|s| matches!(
                        ...
                        MigrationStep::Table(TableOperation::Alter { actions, .. })
                            if actions.iter().any(|a| matches!(
                                a,
                                ColumnAction::SetIdentityKind { name, kind }
                                    if name == "id" && *kind == IdentityKind::ByDefault
                            ))
                    )),
                    "Should have SetIdentityKind action, got: {:?}",
                    steps
                );
