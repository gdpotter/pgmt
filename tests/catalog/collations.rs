...
use anyhow::Result;
use pgmt::catalog::collation::{Collation, CollationProvider};
use pgmt::catalog::domain::Domain;
use pgmt::catalog::id::{DbObjectId, DependsOn};
use pgmt::catalog::raw::collation as raw_collation;
use pgmt::catalog::raw::domain as raw_domain;
use sqlx::postgres::PgConnection;

async fn fetch(conn: &mut PgConnection) -> Result<Vec<Collation>> {
    load_converted(conn, raw_collation::load).await
}

async fn fetch_domains(conn: &mut PgConnection) -> Result<Vec<Domain>> {
...
async fn test_domain_collation_is_schema_qualified() {
    with_test_db(async |db| {
        ...
        let domains = fetch_domains(&mut *db.conn().await).await.unwrap();
