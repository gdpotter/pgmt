...
/// Identity-owned sequences (GENERATED ... AS IDENTITY) are internal to their
/// column — they must not surface as standalone sequences in the catalog or
/// in the lightweight identity snapshot, while serial-owned sequences still do.
#[tokio::test]
async fn test_identity_sequences_are_not_standalone() {
    with_test_db(async |db| {
        db.execute(
            "CREATE TABLE rental (
        ...
        assert!(
            ...
            "serial sequence should be standalone, got: {:?}",
            names
        );
        assert!(
            !names.iter().any(|n| n.contains("rental_id")),
            "identity sequence must be excluded, got: {:?}",
            names
        );

        // The identity snapshot must agree (file-to-object attribution).
        let snapshot = pgmt::catalog::identity::CatalogIdentity::load(db.pool())
            .await
            .unwrap();
        for object in &snapshot.objects {
            if let pgmt::catalog::id::DbObjectId::Sequence { name, .. } = object {
                assert!(
                    !name.contains("rental_id"),
                    "identity sequence leaked into CatalogIdentity: {}",
                    name
                );
            }
        }
