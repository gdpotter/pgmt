...
//! that both baseline-producing paths — `migrate baseline` (checkpoint) and
//! `migrate new --create-baseline` — capture only the user's schemas, never the
//! substrate the image provides. They're slow (large image pulls + init
//! scripts) and need Docker, so they only run when `PGMT_SUBSTRATE_TESTS` is
//! set — CI runs them in
...
/// Which command writes the baseline under test. Both diff the desired state
/// against the shadow's pre-schema base, so both must subtract substrate.
#[derive(Clone, Copy)]
enum BaselineSource {
    /// `migrate baseline` — collapse the migration log into a checkpoint.
    Checkpoint,
    /// `migrate new --create-baseline` — a baseline paired with a migration.
    PairedWithMigration,
}

/// Scaffold a throwaway project with `config_yaml` and `schema_files`, produce a
/// baseline via `source`, and return the single generated baseline's SQL.
fn baseline_for(
    ...
    match source {
        ...
        BaselineSource::PairedWithMigration => {
            let mut cmd = assert_cmd::cargo::cargo_bin_cmd!("pgmt");
...
fn assert_excludes_substrate(baseline: &str) {
    for forbidden in [
        ...
        "storage.objects",
        // The schema files declare no extensions, so any CREATE EXTENSION here
        // is an image-provided one leaking in.
        "CREATE EXTENSION",
    ] {
        assert!(
...
/// A baseline written by `migrate new --create-baseline` must subtract the same
/// substrate the checkpoint does. It was generated by diffing against an empty
/// catalog, so every image-provided extension and schema was re-created in the
/// baseline — and provisioning a fresh target built from the same image then
/// collided with objects the image had already installed.
#[test]
fn test_supabase_substrate_excluded_from_paired_baseline() -> Result<()> {
    if !substrate_tests_enabled() {
        eprintln!("skipping Supabase substrate test (set PGMT_SUBSTRATE_TESTS=1 to run)");
        return Ok(());
    }
...
/// The unscoped counterpart: with no `objects` predicate to name the substrate,
/// a paired baseline still excludes it, because it is subtracted as the
/// shadow's pre-schema state.
#[test]
fn test_supabase_substrate_excluded_from_paired_baseline_without_scoping() -> Result<()> {
    if !substrate_tests_enabled() {
        eprintln!("skipping Supabase substrate test (set PGMT_SUBSTRATE_TESTS=1 to run)");
        return Ok(());
    }
