...
- **Adopting a module onto an existing database:** `pgmt migrate apply --modules X` works when pgmt allows it; if it errors with "requires baseline content", run `pgmt migrate provision --modules X` instead. Adoption can also refuse if the target's other modules aren't caught up ("not caught up to that version") — run the `pgmt migrate apply --modules ...` it names first, then retry. Follow the exact command in each error; don't work around it.
