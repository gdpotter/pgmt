...
pub async fn cmd_migrate_update_specific(
    ...
    if !migration_result.has_changes {
        if is_latest {
            ...
            let empty_migration_content = format!(
                ...
                new_description, config.migration.filename_prefix, new_version, config.migration.filename_prefix, target_migration.version
            );
            ...
            if is_latest {
                ...
                let new_filename =
                    format!("{}{}_{}.sql", config.migration.filename_prefix, new_version, new_description.replace(' ', "_"));
    ...
    if dry_run {
        ...
        std::fs::remove_file(&target_migration.path)?;
        let new_filename = format!("{}{}_{}.sql", config.migration.filename_prefix, new_version, new_description.replace(' ', "_"));
        let new_path = migrations_dir.join(&new_filename);
